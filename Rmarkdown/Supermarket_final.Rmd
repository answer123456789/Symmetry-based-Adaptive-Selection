...
library(doParallel)
source('Functions.R')
cl <- makeCluster(8, outfile = 'market_outs.out')
...
Output_number <- foreach (time = 1:Rep_times, .packages =c('MASS', 'glmnet')) %dopar%  {
  ...
  for (i in 1:length(parameter_q)) {
    ...
    sh = get_conventional_threshold1(S_tol,q_loc_fdr)
    number_times = length(which(S_tol > sh))
    sh = get_conventional_threshold1(SPlus_tol,q_loc_fdr)
    number_plus = length(which(SPlus_tol > sh))
    sh = get_conventional_threshold1(Smin_tol,q_loc_fdr)
    number_min= length(which(Smin_tol > sh))
    fdr_q = get_localfdr_threshold_half(lfdr_points13, lfdr_points_2, lfdr_points_4, q_loc_fdr)
    number_adp = length(which(lfdr_points13<=fdr_q))
    number_table[i,] = c(number_adp, number_times, number_plus, number_min)
  }

  return(list(number_table))
}

discovery_table = matrix(0,11,4)
...
plot_data1 = data.frame(Number=as.vector(discovery_table), Number1=as.vector(discovery_table1), Number2=as.vector(sigma_table1), FDR = factor(rep(parameter_q,4)), Method = factor(rep(c('SAS', 'DSM', 'DM', 'DSA'), each =  length(parameter_q)),levels=c('SAS', 'DSA', 'DM', 'DSM'), labels = c('SAS', 'DSM', 'DM', 'DSA')))

Fig = ggplot(data = plot_data1, aes(x=FDR,y=Number,fill=Method)) +
  geom_bar(stat="identity",position = position_dodge(0.7),width = 0.7, alpha = 0.5, color = 'gray') + 
  scale_fill_manual(values =c('#F8766D','#C77CFF','#7CAE00','#00BFC4')) +
  geom_text(aes(x=FDR,y=Number, label=Number1),position = position_dodge(0.7),size=2.2) +
  geom_text(aes(x=FDR,y=Number-1.8, label=Number2),position = position_dodge(0.7),size=1.8) +
  xlab(expression(q)) + 
...
Fig = ggplot(data = plot_data1, aes(x=FDR,y=Number,fill=Method)) +
  ...
  theme(legend.position = c(0.1,0.8),
        ...
        legend.box.background = element_blank(),
        legend.background = element_rect(fill=rgb(1,1,1,alpha=0.001),colour=NA))
ggsave('Market_bar_twosides.jpg',Fig, width = 9, height = 4, dpi = 400)
```
...
Output_number <- foreach (time = 1:Rep_times, .packages =c('MASS', 'glmnet')) %dopar%  {
  ...
  get_conventional_threshold <- function(S, q_loc_fdr){
    t = sort(abs(S))
    Ta = sapply(t,function(x){(0.5*length(S[S<(-x)]))/max(1,length(S[which(S>x & X1 >0)]))})
    sh = min(t[which(Ta<=q_loc_fdr)]) 
    return(sh)
  }
...
Fig = ggplot(data = plot_data1, aes(x=FDR,y=Number,fill=Method)) +
  ...
  theme(legend.position = c(0.1,0.8),
        ...
        legend.box.background = element_blank(),
        legend.background = element_rect(fill=rgb(1,1,1,alpha=0.001),colour=NA))
ggsave('Market_bar_oneside.jpg',Fig, width = 9, height = 4, dpi = 400)
```
