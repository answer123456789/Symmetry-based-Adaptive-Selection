---
title: "Data Splitting Scenarios (i)-(iv)"
...
Output_fdr_power <- foreach (time = 1:Rep_times, .packages =c('MASS', 'glmnet')) %dopar%  {
  ...
  for (k in 1:length(parameter_proportion)) {
    ...
    Kernel_h0 = function(x){0.3*sum(dtnorm((x[1] - X1_4444)/h0[1])*dtnorm((x[2] - X2_4444)/h0[2]))/(h0[1]*h0[2]*length(X1_4444))}
    f0_hat0 = function(x){
    ...
    Kernel_h1 = function(x){sum(dtnorm((x[1] - X1)/h1[1])*dtnorm((x[2] - X2)/h1[2]))/(h1[1]*h1[2]*length(X1))}
    f_hat = function(x){
...
plot_data1 = data.frame(Power = output_table_power_vec, FDR = output_table_fdr_vec, `Threshold` = rep(c('SAS', 'DM', 'DSA', 'DSM'), length(parameter_proportion)), `delta` = rep(factor(parameter_proportion, level = as.character(parameter_proportion)), each=4))
...
plot_data1$Scenario = 'Scenario (i)'
...
plot_data2 = data.frame(Power = output_table_power_vec, FDR = output_table_fdr_vec, `Threshold` = rep(c('SAS', 'DM', 'DSA', 'DSM'), length(parameter_proportion)), `delta` = rep(factor(parameter_proportion, level = as.character(parameter_proportion)), each=4))
...
plot_data2$Scenario = 'Scenario (ii)'
...
colnames(output_table_fdr_power) = c('Adaptive_fdr', 'Times_fdr', 'Plus_fdr', 'Min_fdr','Adaptive_power', 'Times_power', 'Plus_power', 'Min_power','Parameter_proportion')
...
plot_data3$Scenario = 'Scenario (iii)'
...
parameter_proportion = c(600, 800, 1000, 1200, 1400, 1600)
parameter_magnitude = 1
output_table_fdr = data.frame(matrix(0,4*length(parameter_proportion),length(parameter_magnitude)))
output_table_power = data.frame(matrix(0,4*length(parameter_proportion),length(parameter_magnitude)))
output_table_fdr_power = data.frame(matrix(ncol = 8, nrow = 0))
...
plot_data4 = data.frame(Power = output_table_power[,1], FDR = output_table_fdr[,1], `Threshold` = rep(c('SAS', 'DM', 'DSA', 'DSM'), length(parameter_proportion)), `delta` = rep(factor(parameter_proportion, level = as.character(parameter_proportion)), each=4))
...
plot_data4$Scenario = 'Scenario (iv)'
...
ggsave('DS_1234_1.jpg',Fig1, width = 10.3, height = 5.5, dpi = 400)
```
...
Fig1 = Fig_power1 + Fig_power2 + Fig_power3 +Fig_power4+ Fig_FDR1 + Fig_FDR2 +  Fig_FDR3 +  Fig_FDR4 + plot_layout(ncol = 4, guides = 'collect') & theme(legend.position='bottom')
ggsave('DS_1234.jpg',Fig1, width = 10.3, height = 5.5, dpi = 400)
stopImplicitCluster()
stopCluster(cl)
```
